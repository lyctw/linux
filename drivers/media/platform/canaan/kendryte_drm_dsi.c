...
static void dsi_mipi_init(struct dw_dsi *dsi)
{
    ...
    printk("dsi_mipi_init enter,need add our own code\n");

#if 0 
    Let's add our own dsi initialization code
	/*
	 * count phy params
	 */
...
printk("dsi_encoder_disable enter, need add our own code\n");
	if (!dsi->enable)
		return;
#if 0
...
static void dsi_encoder_enable(struct drm_encoder *encoder)
{
    ...
    printk("dsi_encoder_enable enter, need add our own code\n");
	if (dsi->enable)
		return;
#if 0
...
printk("dsi debug, dsi_encoder_mode_valid enter\n");
	/*
    ...
    printk("dsi debug, dsi_encoder_mode_valid normally exit\n");
			       ...
			       struct drm_encoder *encoder)
{
    ...
    printk("dw_drm_encoder_init enter, possible crtc %d\n", crtc_mask);
	if (!crtc_mask) {
    ...
    printk("dw_drm_encoder_init normally exit\n");
...
static const struct drm_display_mode default_mode = {
	.clock = 278000,
	.hdisplay = 1280,
	.hsync_start = 1280 + 128,
	.hsync_end = 1280 + 128 + 64,
	.htotal = 1280 + 128 + 64 + 64,
	.vdisplay = 800,
	.vsync_start = 800 + 4,
	.vsync_end = 800 + 4 + 8,
	.vtotal = 800 + 4 + 8 + 32,
...
static int dsi_connector_get_modes(struct drm_connector *connector)
{
    ...
    printk("dsi debug, dsi_connector_get_modes enter\n");
    ...
    printk("dsi debug, dsi_connector_get_modes exit normally\n");
    return 1;
...
static int dsi_create_connector(struct drm_device *drm, struct dw_dsi *dsi)
{
	int ret;

	ret = drm_connector_init(drm, &dsi->connector, &dsi_connector_funcs,
	...
	drm_connector_helper_add(&dsi->connector, &dsi_connector_helper_funcs);
	...
	if (dsi->panel) {
        ...
        printk("dsi, attached panel\n");
	}
    else
        printk("dsi, no panel found\n");
    printk("dsi_debug, create connector normally exit\n");

	return 0;

err_connector_cleanup:
	drm_connector_cleanup(&dsi->connector);
    printk("dsi_debug, create connector *ERR* exit\n");
	return ret;
}


static void dsi_destroy_conn_enc(struct dw_dsi *dsi)
{
...
static int dsi_bridge_init(struct drm_device *dev, struct dw_dsi *dsi)
{
	...
	if (ret) {
		...
		//return ret; not return error for debug
		
		/* Otherwise create our own connector and attach to a panel */
		ret = dsi_create_connector(dev, dsi);
		if (ret) {
...
static int dsi_parse_dt(struct platform_device *pdev, struct dw_dsi *dsi)
{
	 ...
	 */
	ret = drm_of_find_panel_or_bridge(np, 1, 0, &dsi->panel, &dsi->bridge);
    printk("dsi_debug, find panel/bridge ret %d\n", ret);
	//if (ret)
	//	return ret;
    ...
    printk("dsi_debug, ctx base 0x%lx\n", ctx->base);
	if (IS_ERR(ctx->base)) {
...
static int dsi_probe(struct platform_device *pdev)
{
    ...
    printk("dsi_debug, dsi_parse_dt ret=%d\n", ret);
	if (ret)
		return ret;

	platform_set_drvdata(pdev, data);

    int status = component_add(&pdev->dev, &dsi_ops);
    printk("dsi_debug, dsi_pfobe done, com add ret=%d\n", status);
	return status;
}
