			     ...
			     struct al5_reg *reg)
{
	...
	// al5_info("DRIVER: al5r_codec_read_register\n");
...
irqreturn_t al5r_hardirq_handler(int irq, void *data)
{
	...
	// al5_info("DRIVER: al5r_hardirq_handler:");

	mask = ioread32(codec->regs + AL5_INTERRUPT_MASK);
	unmasked_irq_bitfield = ioread32(codec->regs + AL5_INTERRUPT);
	irq_bitfield = unmasked_irq_bitfield & mask;
	if (irq_bitfield == 0) {
		al5_dbg("bitfield is 0\n");
		return IRQ_NONE;
	}
	iowrite32(unmasked_irq_bitfield, codec->regs + AL5_INTERRUPT);
	ioread32(codec->regs + AL5_INTERRUPT);
	...
	codec->irq_cnt += 1;//cuiyan
	// al5_info("DRIVER:codec->irq_cnt = %d\n", codec->irq_cnt);
	// al5_info("%d\n", codec->irq_cnt);
