...
#include "../vi.h"
#include "../../k510_isp.h"
#include "vi_pipe_drv.h"
#include "vi_pipe_reg.h"
...
*
*/
 int VI_DRV_PIPE_SetPipeCtl(struct k510_isp_device  *isp,int s8Index, VI_PIPE_CTRL_S *pstPipeCtl)
{
	union U_VI_PIPE_CTRL stData;
	stData.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetTimingCtl(struct k510_isp_device  *isp,int s8Index, VI_PIPE_TIMING_CTRL_S *pstTimingCtl)
{

	union U_VI_PIPE_TIMING_CTRL stData;	
	stData.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetSyncWidthCtl(struct k510_isp_device  *isp,int s8Index, VI_PIPE_SYNC_WIDTH_S *pstSyncWidth)
{
	
	union U_VI_PIPE_SYNC_WIDTH stData;
	stData.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetImageAttr(struct k510_isp_device  *isp,int s8Index, VI_PIPE_IMAGE_ATTR_S *pstImgAttr)
{

	union U_VI_PIPE_IMAGE_SIZE_TOTAL			stImageSizeTotal;
	stImageSizeTotal.u32 = 0; //0x08970464
//	printk("%s:image_h_size_total(%d),image_v_size_total(%d)\n",__func__,pstImgAttr->stImageSizeTotal.image_h_size_total,pstImgAttr->stImageSizeTotal.image_v_size_total);
	...
	union U_VI_PIPE_IMAGE_SIZE_ACTIVE			stImageSizeActive;
	...
	union U_VI_PIPE_IMAGE_START 				stImageStart;	
...
*
*/
int VI_DRV_PIPE_SetBuf(struct k510_isp_device  *isp,int s8Index,VI_PIPE_BUF_S *pstBuf)
{

	union U_VI_PIPE_FRM_BASE_ADDR0_Y stFrameY0;
	stFrameY0.u32 = 0;
	...
	union U_VI_PIPE_FRM_BASE_ADDR1_Y stFrameY1;
	stFrameY1.u32 = 0;
	...
	union U_VI_PIPE_FRM_BASE_ADDR0_UV stFrameUV0;
	stFrameUV0.u32 = 0;
	...
	union U_VI_PIPE_FRM_BASE_ADDR1_UV stFrameUV1;	
	stFrameUV1.u32 = 0;
	...
	union U_VI_PIPE_LINE_STRIDE stFrameStride;
	stFrameStride.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetTpgDmaCtl(struct k510_isp_device  *isp,int s8Index,VI_PIPE_TPG_DMA_CTL_S *TpgDmaCtl)
{
	union U_VI_PIPE_TPG_DMA_CTL stTpgDmaCtl;
	stTpgDmaCtl.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetTpgSize(struct k510_isp_device  *isp,int s8Index,VI_PIPE_TPG_IMAGE_ATTR_S *pstPipeTpgImage)
{

	VI_PIPE_TPG_SIZE_TOTAL_S   *pstTpgSizeTotal = &pstPipeTpgImage->stTpgSizeTotal;
	union U_VI_PIPE_TPG_SIZE_TOTAL stSizeTotal;
	stSizeTotal.u32 = 0;
	...
	union U_VI_PIPE_TPG_SIZE_ACTIVE stSizeAct;
	...
	union U_VI_PIPE_TPG_START stStart;
...
*
*/
int VI_DRV_PIPE_SetTpgBuf(struct k510_isp_device  *isp,int s8Index,VI_PIPE_TPG_BUF_S  *pstPipeTpgBuf)
{

	union U_VI_PIPE_TPG_FRM_BASE_ADDR0_Y stYBuf0;
	stYBuf0.u32 = 0;
	...
	union U_VI_PIPE_TPG_FRM_BASE_ADDR1_Y stYBuf1;
	stYBuf1.u32 = 0;
	...
	union U_VI_PIPE_TPG_LINE_STRIDE stLineStride;
	stLineStride.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetDmaCtrl(struct k510_isp_device  *isp,int s8Index,VI_PIPE_DMA_MODE_S *DmaCtrl)
{
    
	union U_VI_PIPE_DMA_MODE stDmaCtrl;
	stDmaCtrl.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetTpgAttr(struct k510_isp_device  *isp,int s8Index,VI_PIPE_TPG_S *pstTpg)
{
...
*
*/
int VI_DRV_PIPE_SetIspTpg(struct k510_isp_device  *isp,int s8Index, VI_PIPE_TPG_CTL_S *stPipeTpgCtl)
{ 

	union U_VI_PIPE_TPG_CTL stTpgCtl;
	stTpgCtl.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetTofTpg(struct k510_isp_device  *isp,int s8Index,VI_PIPE_TPG_TOF_S *stPipeTpgTof)
{

	union U_VI_PIPE_TPG_TOF stTpgTof;
	stTpgTof.u32 = 0;
...
*
*/
int VI_DRV_PIPE_SetCfgDone(struct k510_isp_device  *isp,int s8Index,VI_PIPE_CONFIG_DONE_S *pstCfgDone)
{

	union U_VI_PIPE_CONFIG_DONE stData;
...
*
*/
int VI_DRV_PIPE_SET_IntPol(struct k510_isp_device  *isp,int s8Index,VI_PIPE_CONFIG_DONE_S *pstCfgDone)
{
	union U_VI_PIPE_CONFIG_DONE stData;
	stData.u32 = isp_reg_readl(isp,reg_vi_pipe_base[s8Index],VI_PIPE_CONFIG_DONE);
	stData.bits.int_pol = pstCfgDone->int_pol;//1;
...
*
*/
int VI_PIPE_DRV_SET_RegWP(struct k510_isp_device  *isp,int s8Index,VI_PIPE_CONFIG_DONE_S *pstCfgDone)
{
...
*
*/
 int VI_DRV_PIPE_SetInt(struct k510_isp_device  *isp,int s8Index,VI_PIPE_INT_CTL_S *pstIntCtl)
{
	union U_VI_PIPE_INT_CTL stIntCtl;
...
*
*/
unsigned int VI_DRV_PIPE_GetPipeIntSts(struct k510_isp_device  *isp,int s8Index)
{
...
*
*/
int VI_DRV_PIPE_ClearPipeIntSts(struct k510_isp_device  *isp,int s8Index,int int_status)
{
...
*
*/
int VI_DRV_PIPE_SetPipeInt(struct k510_isp_device  *isp,int s8Index,int en_int)
{
 ...
 * 
 */
int VI_DRV_PIPE_SetWBuf(struct k510_isp_device  *isp,int s8Index,unsigned long BufAddr)
{
