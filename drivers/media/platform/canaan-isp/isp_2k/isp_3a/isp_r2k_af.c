...
static void r2k_af_setup_regs(struct k510isp_stat *af, void *priv)
{
	...
	isp_reg_writel(af->isp,stAfCtl.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_CTL);

	union U_ISP_CORE_AF_STAT_WIN_H_START stAfWinHStart;
	stAfWinHStart.u32 = 0;
	stAfWinHStart.bits.af_stat_win_h_start = conf->af_win_stth;
	isp_reg_writel(af->isp,stAfWinHStart.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_STAT_WIN_H_START);

	union U_ISP_CORE_AF_STAT_WIN_V_START stAfWinVStart;
	stAfWinVStart.u32 = 0;
	stAfWinVStart.bits.af_stat_win_v_start = conf->af_win_sttv;
	isp_reg_writel(af->isp,stAfWinVStart.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_STAT_WIN_V_START);

	union U_ISP_CORE_AF_STAT_WIN_H_END stAfWinHEnd;
	stAfWinHEnd.u32 = 0;
	stAfWinHEnd.bits.af_stat_win_h_end = conf->af_win_endh;	
	isp_reg_writel(af->isp,stAfWinHEnd.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_STAT_WIN_H_END);

	union U_ISP_CORE_AF_STAT_WIN_V_END stAfWinVEnd;
	stAfWinVEnd.u32 = 0;
	stAfWinVEnd.bits.af_stat_win_v_end = conf->af_win_endv;	
	isp_reg_writel(af->isp,stAfWinVEnd.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_STAT_WIN_V_END);
 ...
 * @param enable 
 */
static void r2k_af_enable(struct k510isp_stat *af, int enable)
{
	union U_ISP_CORE_AF_CTL stAfCtl;
	stAfCtl.u32 = isp_reg_readl(af->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_CTL);
	stAfCtl.bits.af_stat_en = enable;
	isp_reg_writel(af->isp,stAfCtl.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_CTL);
}

static int r2k_af_get_stats(struct k510isp_stat *af, void *priv)
{
    struct k510isp_af_stats *af_stats = priv;

	af_stats->af_mf = isp_reg_readl(af->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_MID_FRQ_DATA);
	af_stats->af_hf = isp_reg_readl(af->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_HIGH_FRQ_DATA);
	af_stats->af_pxl = isp_reg_readl(af->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AF_STAT_PIXEL_NUM);

	return 0;
}
/* Function to check paxel parameters */
static int r2k_af_validate_params(struct k510isp_stat *af, void *new_conf)
{
...
static void r2k_af_set_params(struct k510isp_stat *af, void *new_conf)
{
	struct k510isp_af_config *user_cfg = new_conf;
...
static long r2k_af_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct k510isp_stat *stat = v4l2_get_subdevdata(sd);

	switch (cmd) {
	case VIDIOC_K510ISP_R2K_AF_CFG:
		return k510isp_stat_config(stat, arg);
	case VIDIOC_K510ISP_R2K_AF_STAT_REQ:
		//return k510isp_stat_request_statistics(stat, arg);
		return r2k_af_get_stats(stat, arg);
	case VIDIOC_K510ISP_R2K_AF_STAT_EN: {
		int *en = arg;
...
static const struct k510isp_stat_ops r2k_af_ops = {
	.validate_params	= r2k_af_validate_params,
	.set_params		= r2k_af_set_params,
	.setup_regs		= r2k_af_setup_regs,
	.enable			= r2k_af_enable,
	.get_stats			= r2k_af_get_stats,
};

static const struct v4l2_subdev_core_ops r2k_af_subdev_core_ops = {
	.ioctl = r2k_af_ioctl,
	.subscribe_event = k510isp_stat_subscribe_event,
	.unsubscribe_event = k510isp_stat_unsubscribe_event,
};

static const struct v4l2_subdev_video_ops r2k_af_subdev_video_ops = {
	.s_stream = k510isp_stat_s_stream,
};

static const struct v4l2_subdev_ops r2k_af_subdev_ops = {
	.core = &r2k_af_subdev_core_ops,
	.video = &r2k_af_subdev_video_ops,
};
/**
 * @brief 
...
{
	struct k510isp_stat *af = &isp->isp_r2k_af;
    ...
    {		
        dev_err(af->isp->dev,
			"R2K_AF: cannot allocate memory for configuration.\n");
		return -ENOMEM;
    }
	af->ops = &r2k_af_ops;
	af->priv = af_cfg;
	af->event_type = V4L2_EVENT_K510ISP_R2K_AF;
	af->isp = isp;
	...
	if (!af_recover_cfg) {
		dev_err(af->isp->dev,
			"r2k_AF: cannot allocate memory for recover configuration.\n");
		return -ENOMEM;
	}
    ...
    af_recover_cfg->af_win_endh = K510ISP_WIN_HZSTART_MIN;
    af_recover_cfg->af_win_sttv = K510ISP_WIN_VTSTART_MIN;
    af_recover_cfg->af_win_endv = K510ISP_WIN_VTSTART_MIN;

	if (r2k_af_validate_params(af, af_recover_cfg)) {
		dev_err(af->isp->dev,
	...
	return k510isp_stat_init(af, "r2k_af", &r2k_af_subdev_ops);
}
/**
 * @brief 
 * 
...
{
	k510isp_stat_cleanup(&isp->isp_r2k_af);
}