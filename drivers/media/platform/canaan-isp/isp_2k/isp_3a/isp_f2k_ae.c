...
/*
 * f2k_ae_update_regs - Helper function to update h3a registers.
 */
static void f2k_ae_setup_regs(struct k510isp_stat *ae, void *priv)
{
	...
	isp_reg_writel(ae->isp,stAeCtl.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_CTL);

	union U_ISP_CORE_AE_WIN_H_START stHStart;
	stHStart.u32 = 0;
	stHStart.bits.ae_win_h_start = conf->ae_win_stth;
	isp_reg_writel(ae->isp,stHStart.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_WIN_H_START);

	union U_ISP_CORE_AE_WIN_V_START stVStart;
	stVStart.u32 = 0;
	stVStart.bits.ae_win_v_start = conf->ae_win_sttv;
	isp_reg_writel(ae->isp,stVStart.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_WIN_V_START);

	union U_ISP_CORE_AE_WIN_H_END stHEnd;
	stHEnd.u32 = 0;
	stHEnd.bits.ae_win_h_end = conf->ae_win_endh;
	isp_reg_writel(ae->isp,stHEnd.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_WIN_H_END);

	union U_ISP_CORE_AE_WIN_V_END stVEnd;
	stVEnd.u32 = 0;
	stVEnd.bits.ae_win_v_end = conf->ae_win_endv;
	isp_reg_writel(ae->isp,stVEnd.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_WIN_V_END);

	union U_ISP_CORE_AE_TAR_BR stTarBr;
	stTarBr.u32 = 0;
	stTarBr.bits.ae_tar_bright = conf->ae_yobj;
	isp_reg_writel(ae->isp,stTarBr.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_TAR_BR);

	union U_ISP_CORE_AE_TAR_BR_RANGE stTarBrRange;
	stTarBrRange.u32 = 0;
	stTarBrRange.bits.ae_tar_bright_range = conf->ae_av_rg;
	isp_reg_writel(ae->isp,stTarBrRange.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_TAR_BR_RANGE);

	union U_ISP_CORE_AE_L_EX_TIME stLExTime;
	stLExTime.u32 = 0;
	stLExTime.bits.ae_l_ex_time = conf->ae_exp_l;
	isp_reg_writel(ae->isp,stLExTime.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_L_EX_TIME);

	union U_ISP_CORE_AE_M_EX_TIME stMExTime;
	stMExTime.u32 = 0;
	stMExTime.bits.ae_m_ex_time = conf->ae_exp_m;
	isp_reg_writel(ae->isp,stMExTime.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_M_EX_TIME);

	union U_ISP_CORE_AE_S_EX_TIME stSExTime;
	stSExTime.u32 = 0;
	stSExTime.bits.ae_s_ex_time = conf->ae_exp_s;
	isp_reg_writel(ae->isp,stSExTime.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_S_EX_TIME);

	union U_ISP_CORE_AE_AGC stAgc;
	stAgc.u32 = 0;
	stAgc.bits.ae_agc = conf->ae_agc;
	isp_reg_writel(ae->isp,stAgc.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_AGC);
	...
	isp_reg_writel(ae->isp,stDriveSignalMax.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_APE_DRV_SIGNAL_MAX);

	union U_ISP_CORE_APE_COEFF_DIS stApeCoeffDis;
	stApeCoeffDis.u32 = 0;
	stApeCoeffDis.bits.ape_coeff_distance = conf->apt_ki;
	isp_reg_writel(ae->isp,stApeCoeffDis.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_APE_COEFF_DIS);

	union U_ISP_CORE_APE_COEFF_SPEED stApeCoeffSpeed;
	stApeCoeffSpeed.u32 = 0;
	stApeCoeffSpeed.bits.ape_coeff_speed = conf->apt_kp;
	isp_reg_writel(ae->isp,stApeCoeffSpeed.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_APE_COEFF_SPEED);

	union U_ISP_CORE_APE_COEFF_ACCE stApeCoeffAcce;
	stApeCoeffAcce.u32 = 0;
	stApeCoeffAcce.bits.ape_coeff_acceleration = conf->apt_kd;
	isp_reg_writel(ae->isp,stApeCoeffAcce.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_APE_COEFF_ACCE);
	...
	isp_reg_writel(ae->isp,stApeDriManValue.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_APE_DRV_MANUAL_VALUE);
	...
	isp_reg_writel(ae->isp,stDampManValue.u32,ISP_IOMEM_F2K_CORE,ISP_CORE_APE_DAMP_MANUAL_VALUE);	
...
static void f2k_ae_enable(struct k510isp_stat *ae, int enable)
{
	union U_ISP_CORE_AE_CTL stAeCtl;
	stAeCtl.u32 = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_CTL);
...
static int f2k_ae_get_stats(struct k510isp_stat *ae, void *priv)
{
    struct k510isp_ae_stats *ae_stats = priv;

  	ae_stats->ae_wren = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_VALUE_READY);
  	ae_stats->ae_expl = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_LONG_CUR_EX);
   	ae_stats->ae_expm = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_MID_CUR_EX);
  	ae_stats->ae_exps = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_SHORT_CUR_EX);
  	ae_stats->ae_agco = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_CUR_DIGITAL_GAIN);
  	ae_stats->y_av = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_CUR_AVE_BRIGHTNESS);
  	ae_stats->dn_st = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_CUR_DN_STATUS);
  	ae_stats->ae_exp_st = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_EX_STATUS);
  	ae_stats->ae_sum = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_SUM);
  	ae_stats->ae_pxl = isp_reg_readl(ae->isp,ISP_IOMEM_F2K_CORE,ISP_CORE_AE_PIXEL_SUM);
	  
	return 0;
}

static int f2k_ae_validate_params(struct k510isp_stat *ae, void *new_conf)
{
...
/*
 * f2k_ae_set_params - Helper function to check & store user given params.
 * @new_conf: Pointer to AE and ae parameters struct.
 *
 * As most of them are busy-lock registers, need to wait until AEW_BUSY = 0 to
 * program them during ISR.
 */
static void f2k_ae_set_params(struct k510isp_stat *ae, void *new_conf)
{
	struct k510isp_ae_config *user_cfg = new_conf;
...
static const struct k510isp_stat_ops f2k_ae_ops = {
	.validate_params	= f2k_ae_validate_params,
	.set_params		= f2k_ae_set_params,
	.setup_regs		= f2k_ae_setup_regs,
	.enable			= f2k_ae_enable,
	.get_stats		= f2k_ae_get_stats,
};
/****************************************************************
 * 
 * **************************************************************/
static long f2k_ae_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct k510isp_stat *stat = v4l2_get_subdevdata(sd);

	switch (cmd) {
	    case VIDIOC_K510ISP_F2K_AE_CFG:
	    	return k510isp_stat_config(stat, arg);
	    case VIDIOC_K510ISP_F2K_AE_STAT_REQ:
	    	//return k510isp_stat_request_statistics(stat, arg);
			return f2k_ae_get_stats(stat, arg);
	    case VIDIOC_K510ISP_F2K_AE_STAT_EN: 
        {
...
static const struct v4l2_subdev_core_ops f2k_ae_subdev_core_ops = {
	.ioctl = f2k_ae_ioctl,
	.subscribe_event = k510isp_stat_subscribe_event,
	.unsubscribe_event = k510isp_stat_unsubscribe_event,
};

static const struct v4l2_subdev_video_ops f2k_ae_subdev_video_ops = {
	.s_stream = k510isp_stat_s_stream,
};

static const struct v4l2_subdev_ops f2k_ae_subdev_ops = {
	.core = &f2k_ae_subdev_core_ops,
	.video = &f2k_ae_subdev_video_ops,
};
/****************************************************************
 * 
 ***************************************************************/
/*
 * k510isp_f2k_ae_init - Module Initialisation.
 */
int k510isp_f2k_ae_init(struct k510_isp_device *isp)
{
	struct k510isp_stat *ae = &isp->isp_f2k_ae;
	...
	ae->ops = &f2k_ae_ops;
	ae->priv = ae_cfg;
	ae->event_type = V4L2_EVENT_K510ISP_F2K_AE;
	ae->isp = isp;
	...
	if (!ae_recover_cfg) {
		dev_err(ae->isp->dev,
			"f2k_ae: cannot allocate memory for recover configuration.\n");
		return -ENOMEM;
	}
    ...
    ae_recover_cfg->ae_exp_max = 0x437;
	...
	if (f2k_ae_validate_params(ae, ae_recover_cfg)) {
		dev_err(ae->isp->dev,
	...
	return k510isp_stat_init(ae, "f2k_ae", &f2k_ae_subdev_ops);
}
/*
 * k510isp_f2k_ae_cleanup - Module exit.
 */
void k510isp_f2k_ae_cleanup(struct k510_isp_device *isp)
{
	k510isp_stat_cleanup(&isp->isp_f2k_ae);
}