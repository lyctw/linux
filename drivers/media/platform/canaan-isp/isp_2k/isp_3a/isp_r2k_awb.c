...
/*
 * r2k_awb_update_regs - Helper function to update h3a registers.
 */
static void r2k_awb_setup_regs(struct k510isp_stat *awb, void *priv)
{
	...
	stAwbCtl.u32 = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_CTL);
	...
	isp_reg_writel(awb->isp, stBlueMaxValue.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BLUE_MAX_VALUE);

	union U_ISP_CORE_AWB_RED_MIN_VALUE stRedMinValue;
	stRedMinValue.u32 = 0;
	stRedMinValue.bits.awb_red_min_value = conf->awb_rmin;
	isp_reg_writel(awb->isp, stRedMinValue.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_RED_MIN_VALUE);
	...
	isp_reg_writel(awb->isp, stBlueMinValue.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BLUE_MIN_VALUE);	

	union U_ISP_CORE_AWB_RED_OBJ_VALUE stRedObjValue;
	stRedObjValue.u32 = 0;
	stRedObjValue.bits.awb_red_obj_value = conf->awb_r_obj;
	isp_reg_writel(awb->isp, stRedObjValue.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_RED_OBJ_VALUE);
	...
	isp_reg_writel(awb->isp, stBlueObjValue.u32,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BLUE_OBJ_VALUE);
...
static void r2k_awb_enable(struct k510isp_stat *awb, int enable)
{
	union U_ISP_CORE_AWB_CTL stAwbCtl;
	stAwbCtl.u32 = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_CTL);
...
static int r2k_awb_get_stats(struct k510isp_stat *awb, void *priv)
{
	struct k510isp_awb_stats *awb_stats = priv;

  	awb_stats->bfb_pos = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BLUE_HIST_VALUE);
   	awb_stats->bfb_pot = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BLUE_HIST_PIXEL);
  	awb_stats->rfb_pos = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_RED_HIST_VALUE);
  	awb_stats->rfb_pot = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_RED_HIST_VALUE );
  	awb_stats->bin_pos = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BYPASS_BLUE_HIST_VALUE);
  	awb_stats->bin_pot = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BYPASS_BLUE_HIST_PIXEL);
  	awb_stats->rin_pos = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BYPASS_RED_HIST_VALUE);
  	awb_stats->rin_pot = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BYPASS_RED_HIST_PIXEL);
  	awb_stats->awb_ar = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_RED_VALUE);
  	awb_stats->awb_ag = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_GREEN_VALUE);
  	awb_stats->awb_ab = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_BLUE_VALUE);
  	awb_stats->awb_inr = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_ORG_RED_VALUE);
  	awb_stats->awb_ing = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_ORG_GREEN_VALUE);
  	awb_stats->awb_inb = isp_reg_readl(awb->isp,ISP_IOMEM_R2K_CORE,ISP_CORE_AWB_ORG_BLUE_VALUE);

	return 0;
}

static int r2k_awb_validate_params(struct k510isp_stat *awb, void *new_conf)
{
...
/*
 * r2k_awb_set_params - Helper function to check & store user given params.
 * @new_conf: Pointer to AE and AWB parameters struct.
 *
 * As most of them are busy-lock registers, need to wait until AEW_BUSY = 0 to
 * program them during ISR.
 */
static void r2k_awb_set_params(struct k510isp_stat *awb, void *new_conf)
{
	struct k510isp_awb_config *user_cfg = new_conf;
...
static long r2k_awb_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct k510isp_stat *stat = v4l2_get_subdevdata(sd);

	switch (cmd) {
		case VIDIOC_K510ISP_R2K_AWB_CFG:
			return k510isp_stat_config(stat, arg);
		case VIDIOC_K510ISP_R2K_AWB_STAT_REQ:
			//return k510isp_stat_request_statistics(stat, arg);
			return r2k_awb_get_stats(stat, arg);
		case VIDIOC_K510ISP_R2K_AWB_STAT_EN: 
			{
...
static const struct k510isp_stat_ops r2k_awb_ops = {
	.validate_params	= r2k_awb_validate_params,
	.set_params		= r2k_awb_set_params,
	.setup_regs		= r2k_awb_setup_regs,
	.enable			= r2k_awb_enable,
	.get_stats		= r2k_awb_get_stats,
};

static const struct v4l2_subdev_core_ops r2k_awb_subdev_core_ops = {
	.ioctl = r2k_awb_ioctl,
	.subscribe_event = k510isp_stat_subscribe_event,
	.unsubscribe_event = k510isp_stat_unsubscribe_event,
};

static const struct v4l2_subdev_video_ops r2k_awb_subdev_video_ops = {
	.s_stream = k510isp_stat_s_stream,
};

static const struct v4l2_subdev_ops r2k_awb_subdev_ops = {
	.core = &r2k_awb_subdev_core_ops,
	.video = &r2k_awb_subdev_video_ops,
};

/*
 * k510isp_r2k_awb_init - Module Initialisation.
 */
int k510isp_r2k_awb_init(struct k510_isp_device *isp)
{
	struct k510isp_stat *awb = &isp->isp_r2k_awb;
	...
	awb->ops = &r2k_awb_ops;
	awb->priv = awb_cfg;
	awb->event_type = V4L2_EVENT_K510ISP_R2K_AWB;
	awb->isp = isp;
	...
	if (r2k_awb_validate_params(awb, awb_recover_cfg)) {
		dev_err(awb->isp->dev,
	...
	return k510isp_stat_init(awb, "r2k_awb", &r2k_awb_subdev_ops);
}
/*
 * k510isp_r2k_awb_cleanup - Module exit.
 */
void k510isp_r2k_awb_cleanup(struct k510_isp_device *isp)
{
	k510isp_stat_cleanup(&isp->isp_r2k_awb);
}