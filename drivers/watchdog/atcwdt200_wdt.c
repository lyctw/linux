...
static int wdtdog_open(struct inode *inode, struct file *file){
	...
	for(int i = 0; i < cpu_nums; i++) {
		writel(FLASH_BASE, smu->base +
			  SMU_HARTn_RESET_VEC_LO(i));
		writel(FLASH_BASE >> 32,
			  smu->base + SMU_HARTn_RESET_VEC_HI(i));
	}

	for(int i = 0; i < cpu_nums; i++) {
		u32 lo = readl(smu->base + SMU_HARTn_RESET_VEC_LO(i));
		u32 hi = readl(smu->base + SMU_HARTn_RESET_VEC_HI(i));
		printk("hart%d reset vector (lo32) set to 0x%08x\n", i, lo);
		printk("hart%d reset vector (hi32) set to 0x%08x\n", i, hi);
	}
