...
static int dw_spi_check_status(struct dw_spi *dws, bool raw)
{
	u32 irq_status;
	int ret = 0;
				   ...
				   struct spi_transfer *xfer)
{
	...
	delay.value = nents * dws->n_bytes * 8;
...
static int dw_spi_dma_wait_rx_done(struct dw_spi *dws)
{
	...
	if (ns <= NSEC_PER_USEC) {
		...
		delay.value = clamp_val(us, 0, ((unsigned short)~0U));
	}
